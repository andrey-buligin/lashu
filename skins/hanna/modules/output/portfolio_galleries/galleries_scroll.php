...
print_r($portfolioManager->getThumnailImageXY());
	...
	function showGalleries()
	{
		...
		while ( $portfolioArr = mysql_fetch_assoc($sql_res) )
		{
			if ( $portfolioArr['image_small'] )
			{
				...
				$popupImgSrc = $_CFG['public_images_folder'].'/resized/600_600/'.basename( $image['src'] );
				$ImgSrc      = $_CFG['public_images_folder'].'/resized/300_400/'.basename( $image['src'] );
