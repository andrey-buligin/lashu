...
// [[[ Logika
	...
	$return_from_module = '<ul class="menu level-1" id="menu-nav">
...
// [[[ Funkcii
	...
	function NAV_Generate_HTML($root_category, $start_level, $levels_total = 5, $withSubnavigation = false){
		...
		$total 	 = mysql_result(mysql_query("SELECT count(*) FROM wbg_tree_categories WHERE parent_id=".$root_category." AND enabled=1"),0,0);
		$SQL_str = "SELECT * FROM wbg_tree_categories WHERE parent_id=".$root_category." AND enabled=1 ORDER BY sort_id";
		...
		while ($arr = mysql_fetch_assoc($sql_res))
		{
			...
			$HTML_out .= '<li class="'.implode(" ", $class).'"><a '.getDebug($arr['id']).' href="'.$link.'" '.$hrefClass.'>'.$arr['title'].'</a>'.$subNav.'</li>'."\n";
		}
