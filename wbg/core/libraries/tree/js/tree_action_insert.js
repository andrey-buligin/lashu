/**
*	$id - ID kategorii kotoruju perenosim
*	$target - ID kategorii kuda perenosim
*/
function tree_function_insert($id, $target, $new_subs_in_old, $new_subs_target){

	$old_parent = $categories[$id].$parent_id;

	if (!$categories[$target]){ /* Eto perenos v dr. jazik */
		tree_function_delete($id,  $new_subs_in_old);
		return;
	}
