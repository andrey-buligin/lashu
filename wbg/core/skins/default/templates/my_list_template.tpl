...
function _ml_withselect_onclick($id){
	...
	window.Popup = window.open("'.$this->popup_window['src'].'?edit="+$id+"&'.$this->add_to_get.'","","width=800,height=620,scrollbars=yes,resizable=yes");
}
function _ml_open_copy(){
	if (window.Popup){
		window.Popup.close();
	}
	window.Popup = window.open("'.$_CFG['url_to_cms'].'core/messages/popups/popup.copy_messages.php","","width=800,height=530,scrollbars=yes,resizable=yes");
}
</script>
