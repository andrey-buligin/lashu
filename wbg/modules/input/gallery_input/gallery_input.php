...
function main(){
	...
	TEXTLIST::$sql_table	= "_mod_galleries";
	...
	// <<< Vizov dejstvij s elementami
		...
		if (isset($_GET['saved']) OR isset($_GET['apply'])){
			if (isset($_POST['ins_title']))	{	
				...
				if (@$_POST['img']){
					resizeImages(144, $_POST['img']);
				}
				if (!empty($_POST['_ins_small_image_block_field'])) {
					foreach ($_POST['_ins_small_image_block_field'] as $src) {
						resizeImageTwoTimes($src, 164, 116);
					}
				}	
	...
	if (isset($_GET['edit']) OR isset($_GET['apply'])){
		...
		$js = '<script type="text/javascript" src="/fans/wbg/core/libraries/my_list.class/js/_with_delete.js"></script>
	    <script type="text/javascript" src="/fans/wbg/core/libraries/my_list.class/js/_with_select.js"></script>
	   ';
		return TEXTLIST::show_list($_CFG['current_category']['id']).$js;
	}
}

//======================================================================================

	function resizeImages($neededSize = '', $sourceImg, $src = ''){
		global $_CFG;
		...
		$resizePath = str_replace(dirname($sourceImg['src']),'objects/'.$neededSize.'x'.$neededSize.'', $imgPath);
		
		$newWidth = '';
		$newHeight = '';
		$width  = $sizes[0];
		$height = $sizes[1];
		if ($width > $neededSize OR $height> $neededSize){
			if ($width > $height){
				$newWidth  = $neededSize;
			} else {
				$newHeight = $neededSize;
			}
			exec($_CFG['path_converter']."convert ".$imgPath." -thumbnail ".$newWidth."x".$newHeight." -quality 90 ".$imgPath);
		}
	}
	...
	function resizeImageTwoTimes($imageSrc, $widthNeeded, $heightNeeded){
		...
		$big_img_src2 = $_CFG['path_server'].'images/products/'.basename($imageSrc);
		
		$big_image_param = getimagesize($big_img_src);
		list($width, $height, $type, $attr) = $big_image_param;
	
		if ($width > $widthNeeded) {
			exec($_CFG['path_converter']."convert ".$big_img_src." -thumbnail ".$widthNeeded."x -quality 95 ".$big_img_src);
		}
		if ($height > $heightNeeded) {
			exec($_CFG['path_converter']."convert ".$big_img_src." -thumbnail x".$heightNeeded." -quality 95 ".$big_img_src);
		}	
	}
