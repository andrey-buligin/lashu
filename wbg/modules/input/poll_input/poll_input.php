...
function main(){
	global $_CFG;
	$CFG['SQL_TABLE'] = "_mod_poll_questions";
	$CFG['SQL_TABLE_OPTIONS'] = "_mod_poll_options";
	$ERROR = '';
	...
	// [[[ Sohranjajem dannije esli nado
		...
		if (isSet($_POST['saved_data'])){ // Esli eto bil save dannih , to delajem SAVE
			...
			if ( !$_POST['value_eng'] ) {
				...
				//delete from table
				if ($_GET['edit'] != 0) mysql_query("DELETE FROM _mod_poll_options WHERE ques_id='".$_GET['edit']."'");
				//insert new ones.
				if ( is_array($_POST['value_eng']) ) {
					if ($_GET['edit'] != 0) {
						$ques_id = $_GET['edit'];
					} else {
						$ques_id = 1 + @mysql_result(mysql_query("SELECT id FROM ".$CFG['SQL_TABLE']." ORDER BY id DESC LIMIT 1"),0 ,0);
					}
					foreach ($_POST['value_eng'] as $index => $value_eng) {
						$inserts[] = '('.$ques_id.', "'.$value_eng.'", "'.$_POST['value_lat'][$index].'", "'.$_POST['value_rus'][$index].'")';
					}
					$res = mysql_query("INSERT INTO `_mod_poll_options` (`ques_id`, `value_eng`, `value_lat`, `value_rus`) VALUES ".implode(", ", $inserts));
					echo mysql_error();
				}
			}
...
//		$OUT_filter = $free_filter->generate_filter_form();
		...
		$my_list->insert_cell("question_lat","Question lat", null, "width='25%'");
		$my_list->insert_cell("question_eng","Question eng", null, "width='25%'");
		$my_list->insert_cell("question_rus","Question rus", null, "width='25%'");
